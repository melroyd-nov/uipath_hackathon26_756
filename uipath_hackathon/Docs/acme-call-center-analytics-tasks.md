# Task Plan — Acme Call Center Analytics (UiPath Migration)
...
- `[SME REVIEW]` items still open (carried from the SDDs, to be resolved before or during the corresponding build task): historical PostgreSQL data migration (no task — out of scope per solution overview §1), `AiUsage` Data Fabric upsert pattern (T1/T4), HITL 48h escalation SLA (T5), `LiveCallPage` data source (T7).
